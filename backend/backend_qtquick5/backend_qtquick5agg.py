...
from .backend_qtquick5 import QtCore
from .backend_qtquick5 import QtGui
from .backend_qtquick5 import QtQuick
# from .backend_qtquick5 import FigureManagerQT
# from .backend_qtquick5 import NavigationToolbar2QT
##### Modified Qt5 backend import
from .backend_qtquick5 import FigureCanvasQT
##### not used
from .backend_qtquick5 import show
from .backend_qtquick5 import draw_if_interactive
from .backend_qtquick5 import backend_version
######
...
def new_figure_manager(num, *args, **kwargs):
    ...
    """
    if DEBUG:
        print('backend_qtquick5agg.new_figure_manager')
...
class FigureCanvasQTAggBase(object):
    ...
    def paint(self, p):
        ...
        # First we need to set the canvas size from the QQuickItem size
        w = self.width()
        h = self.height()
        if DEBUG:
...
class FigureCanvasQTAgg(FigureCanvasQTAggBase,
    ...
    def __init__(self, figure):
        ...
        axes.plot([0,1,2,3,4,5], [1,2,7,0,6,2])
        self._drawRect = None
        self.blitbox = None
        # self.setAttribute(QtCore.Qt.WA_OpaquePaintEvent)        
        
        # # Synchronized canvas and QQuickItem size
        # w = self.width()
        # h = self.height()
        # if DEBUG:
            # print('resize (%d x %d)' % (w, h))
